...
Route::get('option-check', 'Front\FrontendController@optionCheck')->name('option-check');
...
Route::group(['middleware' => 'adminlocalize'], function () {
    ...
    Route::prefix('admin')->group(function () {
        ...
        Route::get('check-attr', 'Back\AttributeOptionController@checkAttribute')->name('check-attribute');
        //------------ AUTH ------------
        ...
        Route::group(['middleware' => 'permissions:Manage Products'], function () {
            ...
            Route::get('changeAttribute', 'Back\ItemController@changeAttribute')->name('change-attribute');
            Route::post('item/multi-images', 'Back\ItemController@uploadMultipleImages')->name('back.item.images.multiple');
            Route::get('removeVariant', 'Back\ItemController@removeVariant')->name('removeVariant');
        ...
        // ------------ Manage Warehouse ------------
        Route::group(['middleware' => 'permissions:Manage Warehouses'], function () {
            Route::get('warehouse/add', 'Back\WarehouseController@create')->name('back.warehouse.add');
            Route::post('warehouse/store', 'Back\WarehouseController@store')->name('back.warehouse.store');
            Route::get('warehouse/edit/{id}', 'Back\WarehouseController@edit')->name('back.warehouse.edit');
            Route::put('warehouse/update/{id}', 'Back\WarehouseController@update')->name('back.warehouse.update');
            Route::delete('warehouse/delete/{id}', 'Back\WarehouseController@destroy')->name('back.warehouse.delete');
            Route::get('warehouses', 'Back\WarehouseController@index')->name('back.warehouse.index');
        });
        ...
        Route::group(['middleware' => 'permissions:Manage Categories'], function () {
            ...
            Route::post('category/image', 'Back\CategoryController@categoryImage')->name('back.category.image');
            ...
            Route::post('subcategory/saveImages', 'Back\SubCategoryController@saveImages' )->name('back.subcategory.getImages');
            Route::delete('subcategory/deleteGalleryImage/{id}', 'Back\SubCategoryController@deleteGalleryImage')->name('back.subcategory.deleteGalleryImage');
...
Route::group(['middleware' => 'maintainance'], function () {
    Route::group(['middleware' => 'localize'], function () {
        ...
        Route::prefix('user')->group(function () {
            ...
            Route::post('/login-submit-checkout', 'Auth\User\LoginController@checkoutLogin')->name('user.login.checkout.submit');
            ...
            Route::post('/register-submit-ajax', 'Auth\User\RegisterController@registerAjax')->name('user.register.submit.ajax');
        ...
        Route::get('/cart/destroy/{id}/{var_id?}', 'Front\CartController@destroy')->name('front.cart.destroy');
        ...
        Route::get('products_subcategory/{id}', 'Front\CatalogController@index1')->name('front.catalog.products');
        ...
        // Route::get('/get', 'Front\CatalogController@suggestSearch')->name('front.search.suggest');



        //------------ CHECKOUT ------------
        Route::get('/checkout/page', 'Front\CheckoutController@checkoutPage')->name('front.checkout.page');
        ...
        Route::get('/checkout-stripe_view', 'Front\CheckoutController@stripeView')->name('front.checkout.stripe_view');
        ...
        //------------ Product Attribute Option Details ------------//
        Route::post('/getProductAttributeOptionDetailsMarkup', 'Front\FrontendController@productDefaultAttributeOptionDetails')->name('front.product_attribute_option_details');
