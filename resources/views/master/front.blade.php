...
<head>
    ...
    <link rel="stylesheet" media="screen" href="{{ asset('assets/front/css/mystyle.css') }}">
    @yield('styleplugins')
    ...
    <!-- Color css -->
    <link href="{{ asset('assets/front/css/color.php?primary_color=') . str_replace('#', '', $setting->primary_color) }}"
        rel="stylesheet">
...
">
    ...
    <header class="site-header navbar-sticky">
        @if(url()->current() != url('/checkout/billing/address'))
                                            ...
                                            <ul>
                                                ...
                                                <li class="t-h-dropdown">
                                                    ...
                                                    <div class="t-h-dropdown-menu">
                                                        ...
                                                        @endif
                                                        @foreach (DB::table('pages')->wherePos(0)->orwhere('pos', 2)->get()
    as $page)
        ...
        <div class="navbar">
            <div class="container">
                <div class="row g-3 w-100">
                    ...
                    @php $cats = App\Models\Category::latest()->take(8)->get(); @endphp
                        ...
                        <div class="nav-inner">
                            <nav class="site-menu">
                                <ul>
                                    ...
                                    @foreach($cats as $cat)
                                    @if(count($cat->subcategory) > 0 && !empty($cat->slug))
                                    <li class=""><a
                                                href="{{route('front.catalog').'?category='.$cat->slug}}">{{ $cat->name }}</a></li>
                                    @endif
                                    @endforeach
                                    @if ($setting->is_blog == 1)
            ...
            <div class="row">
                <div class="col-lg-4 col-md-6">
                    ...
                    <section class="widget widget-light-skin">
                        ...
                        <p class="mb-1"><strong>{{ __('Address') }}: </strong>
                            {{ $setting->footer_address }}</p>
                        <p class="mb-1"><strong>{{ __('Phone') }}: </strong>
                            {{ $setting->footer_phone }}</p>
                        <p class="mb-3"><strong>{{ __('Email') }}: </strong>
                            {{ $setting->footer_email }}</p>
                        ...
                        <div class="footer-social-links">
                            @foreach ($links as $link_key => $link)
                                <a href="{{ $link }}" target="_blank"><span><i
                ...
                <div class="col-lg-4 col-sm-6">
                    ...
                    <div class="widget widget-links widget-light-skin">
                        ...
                        <ul>
                            @foreach (DB::table('pages')->wherePos(2)->orwhere('pos', 1)->get()
    as $page)
    ...
    <script>
        function optionCheck(id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#main_price').html("€ " + data.option.price);
                    $('#variant_desc').html(data.option.description);
                    $('#prod_detail').html(data.product_details_markup);
                    $('#product_default_attribute_details_panel').html(data.product_details_markup);
                    // $('#availability').html(data.availability.availability);
                    $('#variable_quantity').val(data.option.variable_quantity);
                    // alert(data.option.price)
                    $('#pricePerMeter').html(data.option.price);
                    $('#variant_id').val(data.option.id);
                    $("#meter_square").val(data.option.variable_quantity);
                    $("#totalPrice").html(data.option.price);
                    $("#packet").val(1);
                    $("#meter_square_value").val(data.option.variable_quantity);
        ...
        $('#option-quantity').on('change', function() {
            ...
            var price = val2[1];
            var p = qty * price;
            $('#option-qty-price').html("$ " + p.toFixed(2));
