                                    ...
                                    <tr>
                                        ...
                                        @if ($wishlist_items->count() > 0)
                                            <th class="text-center"><a class="btn btn-sm btn-primary"
                                                    href="{{ route('user.wishlist.delete.all') }}"><span>{{ __('Clear Wishlist') }}</span></a>
                                            </th>
                                        @endif
                                    </tr>
                                </thead>
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="product-item"><a class="product-thumb"
                                                            href="{{ route('front.product', $product->slug) }}"><img
                                                                src="{{ file_exists(asset('assets/images/' . $product->photo)) ? asset('assets/images/' . $product->photo) : asset('assets/default/no-image.png')  }}"
                                                                alt="Product" class="mt-3"></a>
                                                        <div class="product-info">
                                                            <h4 class="product-title"><a
                                                                    href="{{ route('front.product', $product->slug) }}">{{ $product->name }}</a>
                                                            </h4>
                                                            <div class="text-lg mb-1">
                                                                {{ App\Helpers\PriceHelper::grandCurrencyPrice($product) }}
                                                            </div>
                                                            <div class="text-sm">{{ __('Availability') }}:
                                                                <div
                                                                    class="d-inline text-{{ $product->stock == 0 ? 'danger' : 'success' }}">
