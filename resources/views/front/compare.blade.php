                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <p><b>{{ __('Brand') }}</b> : {{ $items[0]->brand->name }}
                                                , <b>{{ __('Price') }}</b> :
                                                {{ App\Helpers\PriceHelper::grandCurrencyPrice($items[0]) }}
                                            </p>
                                            ...
                                            <p><b>{{ __('Brand') }}</b> : {{ $items[1]->brand->name }}
                                                , <b>{{ __('Price') }}</b> :
                                                {{ App\Helpers\PriceHelper::grandCurrencyPrice($items[1]) }}</p>
                                        </td>
                                        @foreach ($items as $item)
                                            <td>
                                                <div class="comparison-item"><span class="remove-item compare_remove"
                                                        data-href="{{ route('front.compare.remove', $item->id) }}"><i
                                                            class="icon-x"></i></span><a
                                                        class="comparison-item-thumb"
                                                        href="{{ route('front.product', $item->slug) }}"><img
                                                            src="{{ asset('assets/images/' . $item->thumbnail) }}"
                                                            alt="Image"></a><a class="comparison-item-title"
                                                        href="{{ route('front.product', $item->slug) }}">{{ $item->name }}</a><a
                                                        class="btn btn-outline-primary btn-sm add_to_single_cart"
                                                        href="javascript:;"
                                                        data-target="{{ $item->id }}">{{ __('Add to Cart') }}</a>
                                                </div>
                                            </td>
                                        @endforeach
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="comparison-item"><span class="remove-item compare_remove"
                                                    ...
                                                    class="btn btn-outline-primary btn-sm add_to_single_cart"
                                                    href="javascript:;"
                                                    data-target="{{ $item->id }}">{{ __('Add to Cart') }}</a></div>
                                        </td>
                                @endforeach
                                </tr>
