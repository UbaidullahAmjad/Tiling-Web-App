@foreach ($datas as $data)
    <tr>
        ...
        <td>
            @if ($data->type == 'fixed')
                {{ App\Helpers\PriceHelper::adminCurrencyPrice($data->price) }}
            @else
                {{ $data->price }}%
            @endif
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu animated--fade-in" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item"
                        href="{{ route('back.state.status', [$data->id, 1]) }}">{{ __('Enable') }}</a>
                    <a class="dropdown-item"
                        href="{{ route('back.state.status', [$data->id, 0]) }}">{{ __('Disable') }}</a>
                </div>
            </div>
        ...
        <td>
            <div class="action-list">
                <a class="btn btn-secondary btn-sm " href="{{ route('back.state.edit', [$data->id]) }}">
