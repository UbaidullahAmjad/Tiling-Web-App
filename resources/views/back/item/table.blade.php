@foreach ($datas as $data)
    <tr id="product-bulk-delete">
        ...
        <td>{{ $data->id }}</td>
        <td>
            <img src="{{ $data->photo ? asset('assets/images/products/'. $data->id .'/' . $data->photo) : asset('assets/images/placeholder.png') }}"
                alt="Image Not Found">
        </td>
        <td>
            {{ $data->name }}
        ...
        <td>
            <div class="dropdown">
                ...
                <div class="dropdown-menu animated--fade-in" aria-labelledby="dropdownMenuButton">
                    ...
                    @endif
                    @if ($data->status == 1)
                        <a class="dropdown-item" target="_blank" href="{{ route('front.product', $data->slug) }}"><i
                                class="fas fa-angle-double-right"></i> {{ __('View') }}</a>
                    @endif

                    <a class="dropdown-item" href="{{ route('back.item.gallery', $data->id) }}"><i
                            class="fas fa-angle-double-right"></i> {{ __('Gallery Images') }}</a>
                    <!-- <a class="dropdown-item" href="{{ route('back.item.highlight', $data->id) }}"><i
                            class="fas fa-angle-double-right"></i> {{ __('Highlight') }}</a> -->
