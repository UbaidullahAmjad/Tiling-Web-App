        ...
        <div class="card mb-4">
            <div class="card-body">
                <div class="d-sm-flex align-items-center justify-content-between">
                    ...
                    <a class="btn btn-primary btn-sm" href="{{ route('back.code.index') }}"><i
                                ...
                                <form class="admin-form" action="{{ route('back.code.update', $code->id) }}"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" name="no_of_times" class="form-control" id="no_of_times"
                                            placeholder="{{ __('Enter Number Of Times') }}"
                                            value="{{ $code->no_of_times }}" min="1">
                                    </div>
                                                ...
                                                <span class="input-group-text">
                                                    <select name="type" class="form-control">
                                                        <option value="percentage"
                                                            {{ $code->type == 'percentage' ? 'selected' : '' }}>
                                                            {{ __('Percentage') }} (%)</option>
                                                        <option value="amount"
                                                            {{ $code->type == 'amount' ? 'selected' : '' }}>
                                                            {{ __('Amount') }}
                                                            ({{ App\Helpers\PriceHelper::adminCurrency() }})</option>
                                                    </select>
                                                </span>
                                            </div>
                                            <input type="number" id="discount" name="discount" class="form-control"
                                                placeholder="{{ __('Enter Discount') }}" min="0" step="0.1"
