...
$(function ($) {
    ...
    $(document).ready(function () {
        ...
        //     } else {
        //         alert('Minumum Quantity Must Be 1');
        //     }
        ...
        $(document).on("click", ".widget_price_filter", function (e) {
            e.preventDefault();
        ...
        function calculatePrice(clicks,check) {

            // alert(check);exit()
            // var a = $('.input-hidden')[0].checked;
            // alert(a);exit();
            let minOrderQuantity = parseFloat($('#minOrderQuantity').text());
            let variable_quantity = parseFloat($('#variable_quantity').val());
            let avail = parseFloat($('#availability').html());

            // alert(variable_quantity + "========"+minOrderQuantity+ "---------"+avail);exit()
            let qty_value = parseFloat($('.qtyValue').val());
            if(check == 1){
                if(clicks == 1){
                    $('.qtyValue').val(minOrderQuantity);
                }
                else{
                    if (qty_value == variable_quantity) {
                        $('.qtyValue').val(0);
                    }
                    else {
                    // alert(variable_quantity + "========"+qty_value+ "---------"+avail);exit()

                        if((qty_value + variable_quantity) <= avail){
                            $('.qtyValue').val((qty_value) + variable_quantity);
                        }else{
                            $('.qtyValue').val((qty_value));
                            alert('You cannot add more than available items')
                        }
                            ...
                            if((qty_value + variable_quantity) > minOrderQuantity && qty_value != minOrderQuantity){
                                $('.qtyValue').val(qty_value - variable_quantity);
                            }else{
                                $('.qtyValue').val(minOrderQuantity);
                            }
            ...
            let price = parseFloat($('#pricePerMeter').text());
            // alert(variable_quantity);
            // exit()
            let variableQuantity = parseFloat($('.qtyValue').val());
            // alert(variableQuantity);exit
            $('#totalPrice').text(Math.round(price*variableQuantity,2));
        }
        ...
        $(document).on("click", ".increaseQtycart", function () {
            ...
            let attribute_abbrivation = $(this).attr('data-attribute-abbrivation');
            let interval = $(this).attr('data-interval');
            let qty = parseInt($(this).parent().find('input').val()) + ((attribute_abbrivation == "m²") ? 1 : parseInt(interval));
            cartSubmit(item_key,item_id,qty,1);
            getData();
        });
        ...
        $(document).on("click", ".decreaseQtycart", function () {
            ...
            if(qty>0){
                cartSubmit(item_key,item_id,qty,0);
                getData();
            }
        });
        ...
        function cartSubmit(item_key,item_id,cartQty,addremove) {
            
            console.log(item_key,cartQty);
            getData(1,0, item_key,item_id, cartQty,0,addremove);
        };

        function getData(status = 0, check = 0, item_key = 0,item_id = 0,  qty = 0, add_type = 0, addremove = 2) {
            ...
            if (check != 0) {
                ...
                itemId = check;
                variantId = $("#variant_id").val();
                type = 1;
            } else {
                if(indication){
                    itemId = item_id.split("-")[0];
            ...
            var var_id = $('#m_2_id_'+itemId).val();
            
            var p = $('#price_'+itemId).html();
            
            if(packet){
                
                var unit_price = $('#pricePerMeter').html();
                var total_price = $('#totalPrice').html();
                
                
                var meter_square = $('#meter_square').val();

                
                if (status == 1) {
                    let addToCartUrl = `${mainurl}/product/add/cart?item_id=${itemId}&variant_id=${variantId}&unit_price=${unit_price}&total_price=${total_price}&packet=${packet}&meter_square=${meter_square}`;
                    $.ajax({
                        type: "GET",
                        url: addToCartUrl,
                        success: function (data) {
                ...
                var unit_price = $('#un_price_'+itemId).val();
                var total_price = $('#tot_'+itemId).html();
                variantId = var_id;
                if (status == 1) {
                    let addToCartUrl = `${mainurl}/product/add/cart?item_id=${itemId}&variant_id=${variantId}&unit_price=${unit_price}&total_price=${total_price}&packet=${packet}&indication=${indication}&addremove=${addremove}`;
                    $.ajax({
                        type: "GET",
                        url: addToCartUrl,
                        success: function (data) {
                ...
                // alert(variantId);exit()
                let interval = $(".increaseQtycart").attr('data-interval');
                
                if(lit){
                    variantId = var_id;
                    if(addremove == 0){
                        var packet = parseInt($('#it_qty_'+itemId).val()) - parseInt(interval);
                    
                        var unit_price = $('#un_price_'+itemId).val();
                        // var total_price = $('#tot_'+itemId).html();
                        var total_price = unit_price * packet;
                ...
                // alert(total_price+"======="+packet+"======="+unit_price);exit()
                }else{
                    var unit_price = $('#op_price').val();
                    // var packet = $('#option-quantity').find(":selected").text();
                    var packet = $('#op_qty').val();
                    var total_price = unit_price * packet;
                }
                ...
                if (status == 1) {
                    let addToCartUrl = `${mainurl}/product/add/cart?item_id=${itemId}&variant_id=${variantId}&unit_price=${unit_price}&total_price=${total_price}&packet=${packet}&meter_square=`;
                    $.ajax({
                        type: "GET",
                        url: addToCartUrl,
                        success: function (data) {
