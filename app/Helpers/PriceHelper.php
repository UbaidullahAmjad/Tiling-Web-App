...
class PriceHelper
{
    ...
    public static function grandCurrencyPrice($item)
    {
        ...
        if(isset($item->attributeOptions) && !$item->attributeOptions->isEmpty())
            $option_price = $item->attributeOptions[0]->price;
        ...
        if($option_price != 0){
            $chagedprice=explode(",",$option_price);
            if(count($chagedprice) > 2){
                // dd($chagedprice[1]); 
                // dump($chagedprice);
                $concatvalue= $chagedprice[0].".".$chagedprice[1];
                // $concatvalue = 123;
            }
            else{
                // dd($item->attributeOptions);
                  $concatvalue= (float) $item->attributeOptions[0]->price;
            }
        }
   
        
        // $chagedprice=explode(",",$option_price)[0].".".explode(",",$option_price)[1];
       
        // dump($item->discount_price);
        if($item->discount_price == null)
        {
            $price = $concatvalue ;
        }
        else{
            $price = $item->discount_price + $concatvalue ;
        }
        ...
        // dump($price);
        // dd($setting->currency_direction);
        // dd($concatvalue);
        if($setting->currency_direction == 1){
            // dd($price);
            // dump($curr->sign . $price);
         return $price;
         
        }else{
            // dd("klkjfdgl", $curr->sign .$price);
            return $curr->sign . $price;
        //  return  $price.$curr->sign;
        }
    ...
    public static function OrderTotalChart($order)
    {
        ...
        $grand_total = $grand_total - ($discount ? $discount['discount'] : 0);
        $curr = Currency::where('is_default',1)->first();
        $total_amount = round($grand_total * ($curr ? $curr->value : 1),2);
